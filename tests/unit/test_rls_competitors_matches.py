"""RLS DDL render tests for the competitor/match tables (SPEC-05 T008, FR-001/FR-002/SC-007).

Pure string assertions against `emit_rls_policy(...)` for `competitors`
and `competitor_product_matches` — no database. Mirrors
`tests/unit/test_rls_catalog.py` (SPEC-04) by proving both applications
render ENABLE + FORCE + the fail-closed
`NULLIF(current_setting('app.workspace_id', true), '')::uuid` predicate
— exactly what the migration
(`alembic/versions/f4c8a391d5c9_competitors_matches_tables.py`) executes.
"""
...
@pytest.mark.parametrize("table_name", COMPETITORS_MATCHES_TABLES)
...
def test_both_tables_have_distinct_policy_names() -> None:
    policy_stmts = [emit_rls_policy(table_name)[2] for table_name in COMPETITORS_MATCHES_TABLES]
    for table_name, stmt in zip(COMPETITORS_MATCHES_TABLES, policy_stmts):
        assert f"CREATE POLICY {table_name}_workspace_isolation ON {table_name}" in stmt
    assert len({stmt for stmt in policy_stmts}) == len(COMPETITORS_MATCHES_TABLES)
