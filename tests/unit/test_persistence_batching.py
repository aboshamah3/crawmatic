...
class _FakeSettings:
    """Stand-in for `app_shared.config.Settings` -- the SPEC-09 T029
    recompute-dedup field plus the two SPEC-12 US5 T037 fields
    `_flush_batch` now also reads unconditionally (the stats-buffer
    recording gate), no real env/pydantic validation. None of this
    file's seeded `ScrapeResult`s carry a `domain_strategy_profile_id`,
    so `record_attempt` itself is never actually invoked here -- only
    the attribute reads need satisfying."""
