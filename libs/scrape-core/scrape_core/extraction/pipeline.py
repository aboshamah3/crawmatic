...
in order, first hit wins. This US1 slice wires only JSON-LD; US3 (T034)
appends CSS then regex — ``_STRATEGIES`` is the single extension point
so growing the chain never touches the loop itself.
"""
...
# Ordered JSON-LD -> CSS -> regex chain (contracts/extraction.md). Only
# JSON-LD exists in US1; US3 appends `extraction.css.extract_css` and
# `extraction.regex.extract_regex` here, in that order.
_STRATEGIES: tuple[_Strategy, ...] = (extract_jsonld,)
