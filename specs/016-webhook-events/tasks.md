...
- [ ] T009 Add both models to `WORKSPACE_OWNED_MODELS` in `libs/shared/app_shared/repository.py` (single source of truth). Do NOT edit `scripts/check_workspace_scoping.py` — it imports `WORKSPACE_OWNED_MODELS` automatically (no hardcoded list to mirror); just run it afterward to VERIFY the guard stays green.
