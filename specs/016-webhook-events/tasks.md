...
- **REUSE verbatim** (no new/divergent copy): SSRF validator `app_shared.url_safety.validate_competitor_url`; Fernet `app_shared.security.encryption.encrypt_secret` / `has_secret` derivation; keyset cursors in `app_shared.pagination`; `app_shared.messaging.enqueue`; SPEC-15 maintenance machinery.
...
- [ ] T001 VERIFY (no edit) that `WEBHOOKS_READ = "webhooks:read"` and `WEBHOOKS_WRITE = "webhooks:write"` already exist in `libs/shared/app_shared/security/scopes.py` (Scope enum ~lines 31–32) and are referenced by `tests/unit/test_scopes.py`. If present, make NO change; if somehow absent, stop and flag — do not silently re-add.
...
- [ ] T003 VERIFY the current single Alembic head is `4a1dca402f78` by running `uv run alembic heads` (expect exactly one head) so the new migration's `down_revision` is correct; record the value for T009.
- [ ] T004 Add two StrEnums to `libs/shared/app_shared/enums.py`: `WebhookEventStatus` (`PENDING`/`DELIVERED`/`FAILED`; v1 writes only `PENDING`) and `WebhookEventType` with exactly the 8 members from data-model.md (`PRICE_ALERT_CREATED="price.alert.created"`, `PRICE_ALERT_UPDATED="price.alert.updated"`, `PRICE_ALERT_RESOLVED="price.alert.resolved"`, `PRICE_ALERT_REOPENED="price.alert.reopened"`, `SCRAPE_JOB_COMPLETED="scrape.job.completed"`, `SCRAPE_JOB_PARTIAL="scrape.job.partial_failed"`, `SCRAPE_JOB_FAILED="scrape.job.failed"`, `DOMAIN_STRATEGY_UPDATED="domain.strategy.updated"`).
- [ ] T005 [P] Add `CREATE_WEBHOOK_EVENT = "webhook_events.create_webhook_event"` to `libs/shared/app_shared/task_names.py`.
- [ ] T006 [P] Unit test the enums in `tests/unit/test_webhook_enums.py`: assert both StrEnums' exact member→value mappings, that `WebhookEventType` has exactly 8 members with the strings above, and that `WebhookEventStatus.PENDING == "PENDING"` (StrEnum value equality).
...
- [ ] T007 Create `libs/shared/app_shared/models/webhooks.py` with two models (mirror precedents named in research.md R3):
...
- [ ] T033 [US3] Add the SPEC-09 seam in `apps/workers/app/workers/tasks_analysis.py::recompute_variant`: strictly AFTER the existing `session.commit()` (~line 380) and only when `event_type is not None`, build the payload via `build_alert_event(...)` and `enqueue(CREATE_WEBHOOK_EVENT, queue="webhook_events", kwargs={...})`, wrapped in a narrow `try/except` that logs and continues (broker error must never fail the committed op — FR-009/SC-005).
- [ ] T034 [US3] Add the SPEC-08 seam in `apps/workers/app/workers/tasks_jobs.py::finalize_jobs`: collect finalized `(job_id, status)` for terminal statuses (COMPLETED/PARTIAL_FAILED/FAILED; not CANCELLED) in the loop, then AFTER the single `session.commit()` (~line 401) `enqueue` one `create_webhook_event` per finalized job via `build_job_event(...)`, wrapped in the same narrow try/except.
- [ ] T035 [US3] Add the SPEC-12 seam in `apps/workers/app/workers/tasks_strategy.py::flush_stats`: AFTER `session.commit()` (~line 750) enqueue via `build_strategy_event(...)` when `apply_promotion` returned `promoted=True` (→ ACTIVE) and on `apply_rediscovery` (→ DEGRADED), wrapped in the same narrow try/except.
- [ ] T036 [P] [US3] Unit test `tests/unit/test_webhook_enqueue_seams.py`: with `enqueue` and `get_session` monkeypatched/mocked (no live Celery/DB), assert each seam calls `enqueue(CREATE_WEBHOOK_EVENT, queue="webhook_events", kwargs=...)` exactly once with the expected `event_type`/payload for a transition, that `event_type is None` / `UNCHANGED` / `CANCELLED` enqueues nothing, and that a raised broker error inside the seam is swallowed (source path completes).
...
- [ ] T040 [P] Run the workspace-scoping guard `uv run python scripts/check_workspace_scoping.py` (and `tests/unit/` scoping tests) — confirms `WebhookEndpoint`/`WebhookEvent` are in `WORKSPACE_OWNED_MODELS` and the script mirror is in sync.
