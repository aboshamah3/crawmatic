...
- [X] T017 [P] [US1] Create `libs/scrape-core/scrape_core/extraction/__init__.py` and `libs/scrape-core/scrape_core/extraction/result.py` — `ExtractionCandidate` dataclass (`raw_price_text`, `currency`, `method`, `confidence`, `selector_used`, `raw_title`, `stock`, `matched_text`) per contracts/extraction.md.
- [X] T018 [P] [US1] Create `libs/scrape-core/scrape_core/extraction/jsonld.py` — pure `parsel` parse of `<script type="application/ld+json">` `Product`/`Offer` → `ExtractionCandidate` (default confidence 0.95 from `app_shared.profiles.confidence`, never a literal). Depends on T017.
- [X] T019 [US1] Create `libs/scrape-core/scrape_core/extraction/pipeline.py` — ordered `extract(html, profile)` orchestrator (JSON-LD first for this story; CSS/regex added in US3), returns the first-hit candidate else `PRICE_NOT_FOUND`. Depends on T017, T018.
- [X] T020 [US1] Create `libs/scrape-core/scrape_core/validation.py` — `validate_candidate(candidate, validation_rules, confidence_cfg) -> Accepted | Rejected(error_code)` core path: `app_shared.money.parse_money` (exact `Decimal`; reject float/NaN/Infinity/over-scale — never round → `INVALID_PRICE_FORMAT`), `> 0` guard, and the confidence gate (`>= min_accepted_confidence`, default 0.75 via `resolve_confidence_rules`, else `LOW_CONFIDENCE_PRICE`). Full rules extended in US3. Depends on T017.
- [X] T021 [US1] Create `libs/scrape-core/scrape_core/pipelines.py` — `BatchedPersistencePipeline`: buffer `ScrapeResult` items, flush at `SCRAPE_FLUSH_MAX_ITEMS` **or** `SCRAPE_FLUSH_INTERVAL_SECONDS` (Twisted `LoopingCall`) + a final flush at `close_spider`; each flush a **single** `run_in_thread` transaction that bulk-inserts observations + attempts and upserts `match_current_prices` (`insert(...).on_conflict_do_update` on `(workspace_id, match_id)`, never overwriting the current price with a failure). Per contracts/persistence-pipeline.md. Depends on T011, T012, T007.
- [X] T022 [US1] Create `apps/scrapers/price_monitor/spiders/generic_price_spider.py` — parse args (`workspace_id`, `scrape_job_id`, `match_ids`, `mode`); load matches **scoped to `workspace_id`** (skip a match not found for the workspace); consume the **cached resolved** scrape profile (SPEC-06, no per-match re-resolution); yield `DIRECT_HTTP` requests; run extraction + validation in `parse`; yield `ScrapeResult` items for both success and failure (persist-only, no `price_analysis` emission). Per contracts/spider-args.md. Depends on T019, T020, T012.
- [X] T023 [US1] Extend `apps/scrapers/price_monitor/settings.py` — set `ROBOTSTXT_OBEY=False`, register `BatchedPersistencePipeline` in `ITEM_PIPELINES`, wire the small per-process pool, and read flush knobs from `Settings`/config. Depends on T021.

### Tests for User Story 1

- [X] T024 [P] [US1] Create `tests/unit/test_extraction_jsonld_css_regex.py` — JSON-LD fixture extracts with confidence 0.95; orchestrator returns JSON-LD first; `PRICE_NOT_FOUND` when nothing matches (CSS/regex/single-number cases added in US3).
...
**Purpose**: Remaining unbuilt/incorrect work found by `/speckit-converge` assessing the
implemented code against `spec.md`/`plan.md`. Append-only; complete via `/speckit-implement`.
Scope unchanged — no later-spec work (Scope Guardrails still apply).

- [X] T053 [US2] Make the connect-time `SafeResolver` SSRF rejection surface as `BLOCKED`, not `UNKNOWN_ERROR`, per FR-005 / US2 Acceptance Scenario 1 (contradicts). The runtime resolved-IP SSRF check is enforced solely by `libs/scrape-core/scrape_core/safety/resolver.py::SafeResolver` (`validate_competitor_url` in the `SsrfGuardMiddleware` path does not resolve hostnames; `validate_resolved_target` is the test-only injectable seam). Its `UnsafeResolvedAddressError` carries no `error_code` and is wrapped by Twisted (into `scrapy.exceptions.CannotResolveHostError`/`DNSLookupError`) before reaching the spider `errback`, so `scrape_core.errors.classify_exception` returns `UNKNOWN_ERROR`. Fix: (a) set `error_code = ScrapeErrorCode.BLOCKED` on `UnsafeResolvedAddressError` in `safety/resolver.py`; and (b) extend `classify_exception` in `libs/scrape-core/scrape_core/errors.py` to walk the wrapped exception's `__cause__`/`__context__` chain (and/or recognize `UnsafeResolvedAddressError` by name) so a rebinding rejection is classified `BLOCKED` while genuine DNS failures still map to `DNS_ERROR`. Add a `tests/unit/test_fetch_url_safety.py` (or `errors`) case asserting a resolver-path rejection → `BLOCKED`, and un-skip/extend the SSRF live assertion in `tests/integration/test_spider_ssrf_live.py`.

- [X] T054 [US2] Thread the resolved per-competitor `robots_policy` onto each request so `RobotsPolicyMiddleware` honors it per FR-006 (partial). `apps/scrapers/price_monitor/spiders/generic_price_spider.py::start()` sets only `match_id`/`download_slot` on `request.meta`, so `RobotsPolicyMiddleware.process_request` falls through to the `RESPECT` default for every request and a competitor configured `IGNORE_AFTER_APPROVAL`/`REVIEW_REQUIRED` (`Competitor.robots_policy`, an existing column) is never honored — per-competitor robots resolution is not actually wired. Fix: load `Competitor.robots_policy` in `load_targets` (the competitor rows are already fetched there), carry it on `SpiderTarget`, and set `request.meta["robots_policy"] = target.robots_policy` in `start()`; add a spider-level unit assertion that the resolved policy reaches `request.meta`.

- [X] T055 Hoist the duplicated resolution-cache **value codec** into `libs/shared/app_shared/profiles/resolution.py` so both call sites share one definition (unrequested / maintainability drift risk). The cache **key** is already shared (`resolution_cache_key`), but `_encode_group_result`/`_decode_group_result` + `_CACHE_NONE_MARKER`/`_CACHE_FIELD_SEP` are copied verbatim between `apps/api/app/services/profile_resolution.py` and `apps/scrapers/price_monitor/spiders/generic_price_spider.py`; if either format drifts the spider silently misreads SPEC-06's warm cache. Fix: move the encode/decode helpers (and marker/sep constants) into `app_shared.profiles.resolution` and import them from both call sites (respecting `apps → libs` only); keep behavior byte-identical so existing cache entries still decode.
