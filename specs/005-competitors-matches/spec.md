...
- **FR-010**: The system MUST produce, for every match, two distinct derivations that MUST NOT be conflated:
  - **Normalized competitor URL** (the identity value used in the match unique key): lowercase scheme and host, remove `www.`, remove the default port, remove the URL fragment, remove a trailing slash — but **keep** the scheme and **keep** the query string (they distinguish the target product, e.g. `?variant=123`).
  - **Derived URL pattern** (the versioned grouping key): starting from the host+path, drop the scheme and drop the query string, preserve locale prefixes (e.g. `/ar/`, `/en/`), split the path into segments, replace id-like segments (all-digits / UUID-like / long mixed-alphanumeric / mostly-digits) with an id placeholder, and replace product-slug segments after known product path keys (`/products/`, `/product/`, `/p/`, `/item/`, locale-prefixed variants) with a wildcard.
  - The concrete id-like thresholds are plan-level (behind the algorithm version, per Clarifications Q3).
