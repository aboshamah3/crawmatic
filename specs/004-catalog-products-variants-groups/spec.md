...
- **FR-006**: A product MUST always retain at least one variant. This is a structural invariant maintained by the catalog service layer (the default-variant creation + `ensure-at-least-one` logic). Note: this feature exposes **no variant-deletion operation** (variants support list/get/update/bulk-upsert only), so no current endpoint can drop a product to zero variants; the invariant is enforced now by the ensure-at-least-one logic (unit-tested) and will gate any future variant-removal path. It is not an ambiguous runtime check on an operation that cannot occur.
...
- **FR-014**: The system MUST expose the catalog endpoints under `/v1` — products (create, list, get, update, delete, bulk-upsert), variants (list, get, update, bulk-upsert), and product groups (create, list, get, update, delete, add item, remove item) — and nothing outside catalog scope.
