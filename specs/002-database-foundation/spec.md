...
- What happens when a migration fails partway through? Each migration runs inside a transaction so that failing (transactional) DDL rolls back and leaves the database at the prior revision rather than a half-applied state; a downgrade path is provided for each revision. (Any future migration that must use non-transactional statements such as `CREATE INDEX CONCURRENTLY` documents that it is not atomic — no such migration exists in this foundation spec.)
...
- **FR-016**: Each migration MUST run inside a transaction so a failed (transactional) migration rolls back to the prior revision rather than leaving a half-applied schema, and MUST provide a downgrade path; any migration requiring non-transactional statements (e.g. `CREATE INDEX CONCURRENTLY`) MUST document its non-atomicity (none exist in this foundation spec).
