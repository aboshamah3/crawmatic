...
**Purpose**: Close the one genuine, statically-detectable, in-scope gap found by `/speckit-converge` after Phases 1–7. All DB-independent deliverables are green (47 passed, 7 skipped); the four live-Postgres/Docker items (T021, T033, T034, T036) remain intentionally DEFERRED and are NOT re-listed here. Only actionable remaining work is appended below.

- [X] T038 Gate the one-shot `migrate` service behind a compose profile so a default `docker compose up` does NOT start it: add `profiles: ["migrate"]` to the `migrate` service in `/srv/crawmatic/crawmatic/docker-compose.yml` (keep `restart: "no"`, the direct-to-`postgres:5432` `MIGRATION_DATABASE_URL`, and `depends_on: postgres: {condition: service_healthy}` unchanged). Rationale: as wired in T031 the service has no `profiles:` restriction, so on a live Docker host `docker compose up` starts it as a 9th service and runs migrations implicitly — this (a) contradicts FR-011's "dedicated one-shot job" intent (migrations should be explicitly invoked, not part of the default bring-up) and (b) breaks SPEC-01's `tests/integration/test_compose_smoke.py`, which asserts EXACTLY 8 services reach running/healthy (`seen_services == EXPECTED_SERVICES`). Under a `migrate` profile the default `up` stays at the 8 long-running services (SPEC-01 invariant holds) while the job runs only when explicitly invoked. The already-documented runbook command `docker compose run --rm migrate` (quickstart.md §69) continues to work unchanged, since `compose run` starts the named service regardless of profile gating; also runnable as `docker compose --profile migrate up migrate`. Confirm `tests/integration/test_no_startup_migrations.py` still passes (it parses compose statically and does not depend on the profile). If any prose in quickstart.md §B needs it, note that `migrate` is profile-gated. Per FR-011 (partial — cross-spec: preserves SPEC-01 SC-001 8-service topology).
