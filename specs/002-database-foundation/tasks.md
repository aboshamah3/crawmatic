...
- [ ] T015 [P] [US2] Create `tests/unit/test_enums.py`: assert `RecordStatus` members carry their string value, `enum_column` renders a plain `String` column (not a PG-native ENUM and not a DB `Enum`), and an out-of-set value is rejected by the application-side validator (FR-006). [analyze A2]
