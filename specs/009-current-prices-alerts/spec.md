...
- **FR-013**: `price_analysis` MUST upsert `variant_price_states` (benchmarks, comparable_competitor_count, latest_alert_type/severity, calculated_at) and upsert `variant_alert_states`, and MUST write a `price_alert_events` row only when the alert type or severity changes: CREATED on first alert, UPDATED on change, RESOLVED when returning to NORMAL/NONE, REOPENED when leaving NORMAL after a prior resolution, UNCHANGED (or no event) when identical — maintaining first_seen_at/last_seen_at/resolved_at accordingly.
...
- **Alert-state status**: `variant_alert_states.status` follows an active/resolved lifecycle (an alert is active while its type is non-NORMAL and resolved when the variant returns to NORMAL/NONE); the precise status vocabulary is confirmed against the model conventions in planning.
