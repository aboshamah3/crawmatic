...
- [X] T003 [P] Extend `libs/shared/app_shared/task_names.py` with `PRICE_ANALYSIS_RECOMPUTE = "price_analysis.recompute_variant"` (plain string; the module stays celery-free). (FR-012, D4)
- [X] T004 [P] Create `libs/shared/app_shared/alerts/__init__.py` — empty package init for the framework-agnostic engine module (the engine + re-exports land in US1 T014). (D1)
...
- [X] T036 [P] ⏸ DEFERRED (needs live Postgres) Author `tests/integration/test_alerts_isolation_live.py` (Isolation/SC-008) — cross-workspace read blocked (app scoping + RLS) on `variant_price_states`/`variant_alert_states`/`price_alert_events`; no workspace context ⇒ 0 rows from each; missing `alerts:read` scope ⇒ 403.
