...
- [ ] T013 [P] Create `libs/shared/app_shared/config.py` — pydantic-settings `Settings` enumerating every variable in `contracts/environment.md` (DATABASE_URL, DB_POOL_SIZE, DB_MAX_OVERFLOW, REDIS_URL, SCRAPYD_HTTP_URLS, SCRAPYD_BROWSER_URLS, SCRAPYD_USERNAME, SCRAPYD_PASSWORD, API_PORT/PORT, API_PUBLIC_BASE_URL, INTERNAL_API_BASE_URL); fail fast with a clear error on missing required vars; parse `SCRAPYD_*_URLS` as comma-separated pools even when length 1 (FR-017, FR-018)
...
- [ ] T027 [P] [US1] Create `apps/scrapers-browser/scrapyd.conf` — `http_port = 6800`, `bind_address = ::`, basic auth from env, low `max_proc`/concurrency for browser (FR-009, FR-012, FR-016)
- [ ] T028 [P] [US1] Create `apps/scrapers-browser/price_monitor_browser/__init__.py`, `.../settings.py` (scrapy-playwright download handler, low browser concurrency; imports `scrape_core`), and `.../spiders/__init__.py` (FR-004, FR-009)
- [ ] T029 [P] [US1] Create `apps/api/Dockerfile` — base `python:3.13.5-slim-bookworm`, copy `uv` from `ghcr.io/astral-sh/uv:0.7.13`, `uv sync --package api --no-dev`, create + `USER` non-root, CMD `uvicorn app.main:app --host :: --port $PORT` (FR-014, FR-015, FR-016, FR-019)
...
- [ ] T037 [US2] Confirm dual-stack binds are wired across artifacts: `api` command `--host ::` (T029/T034), both `scrapyd.conf` `bind_address = ::` (T024/T027), and `pgbouncer` `listen_addr = *`/`::` in `docker-compose.yml` env (FR-016)
...
- [ ] T040 [P] [US3] Create `.env.example` at repo root enumerating EVERY variable in `contracts/environment.md` with non-secret placeholders: `DATABASE_URL` (host `pgbouncer:6432`), `DB_POOL_SIZE`, `DB_MAX_OVERFLOW`, `REDIS_URL`, `SCRAPYD_HTTP_URLS`, `SCRAPYD_BROWSER_URLS`, `SCRAPYD_USERNAME`, `SCRAPYD_PASSWORD`, `API_PORT`/`PORT`, `API_PUBLIC_BASE_URL`, `INTERNAL_API_BASE_URL`, `POSTGRES_USER`, `POSTGRES_PASSWORD`, `POSTGRES_DB`, `PGBOUNCER_AUTH_TYPE` (`trust` local), `PGBOUNCER_POOL_MODE` (`transaction`); no real secrets (FR-017, SC-006)
