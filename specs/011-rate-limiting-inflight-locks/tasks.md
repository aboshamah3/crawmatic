...
- [ ] T028 [US3] Update the re-dispatch expansion query in `apps/workers/app/workers/tasks_jobs.py` `dispatch_job` (the expansion/target-selection query at ~line 212): change `ScrapeJobTarget.status == PENDING` to `ScrapeJobTarget.status.in_((ScrapeTargetStatus.PENDING, ScrapeTargetStatus.DEFERRED))` so overflowed targets are re-picked and transition `DEFERRED → STARTED`, re-subject to the lock+limiter gate. Do NOT touch the stalled-target reaper query (~line 352) — it must keep matching only its own status set so DEFERRED is not treated as stalled. Leave `_TERMINAL_JOB_STATUSES` guard unchanged (overflow-loop ceiling) (FR-019, US3 AS3).
