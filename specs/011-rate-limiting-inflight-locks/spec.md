...
- **FR-002**: Rate-limit state MUST be keyed by workspace, domain, and access method using the keys `rate:{workspace_id}:{domain}:DIRECT_HTTP`, `rate:{workspace_id}:{domain}:PROXY_HTTP`, and `rate:{workspace_id}:{domain}:PLAYWRIGHT_PROXY`. A retry over the direct transport (`DIRECT_HTTP_RETRY`) MUST reuse the `DIRECT_HTTP` bucket/semaphore rather than a separate key — a retried request still counts against the same physical direct-HTTP budget for the domain. (This slice only issues `DIRECT_HTTP`; the mapping is stated so the retry/proxy/browser transports added by later specs key correctly.)
...
- **FR-006**: When permission is denied, the limiter MUST return a wait hint, and the caller MUST reschedule the request after `max(wait_hint, cooldown_seconds) + bounded random jitter` (e.g. 2–20s) rather than retrying immediately. The domain's configured `cooldown_seconds` (from `DomainAccessRule`, when present) acts as a lower bound on the post-denial wait, so a domain with a mandated cool-down is never re-hit faster than that; when unset it contributes no floor (0).
