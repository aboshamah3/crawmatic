...
| **VI. Internal-Only & Legally Compliant Access (NON-NEGOTIABLE)** | PASS | Adds coordination *before* the fetch; introduces no new access method and no external service. Keys cover exactly the four internal `AccessMethod`s (`DIRECT_HTTP`/`PROXY_HTTP`/`PLAYWRIGHT_PROXY` reserved). SSRF and the existing fetch guard are untouched. On Redis outage the limiter **denies** (never fetches uncontrolled — FR-023), strictly *tightening* access. No raw HTML/screenshot storage introduced. |
