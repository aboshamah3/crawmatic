...
Main source of truth for configuration, products/variants, matches, jobs, observations, current prices, alerts, strategy profiles, and events.

### redis

Used for Celery broker, distributed locks, distributed rate limits, in-flight scrape locks, scheduler lock, and domain semaphores. Celery result backend is not required by default because job state lives in PostgreSQL.
...
price-monitor/
  ...
  specs/
    ...
    15-partitioning-retention-rollups/
    16-webhook-events/
...
```text
worker-service → scrapyd-http-service
worker-service → scrapyd-browser-service
api-service → postgres
worker-service → postgres
scheduler-service → postgres
scrapy services → postgres
...
SCRAPYD_HTTP_URL=http://scrapyd-http-service.railway.internal:6800
SCRAPYD_BROWSER_URL=http://scrapyd-browser-service.railway.internal:6800
...
2. Celery calls Scrapyd’s schedule API.
3. Scrapyd runs the Scrapy spider in its own process.
4. The spider reads configuration from PostgreSQL.
5. The spider writes observations/current prices/request attempts to PostgreSQL.
6. The worker or spider triggers price analysis.
...
2. Resolve scrape profile.
3. Resolve access policy.
4. Resolve domain strategy profile.
5. Use distributed domain rate limiter.
...
Strategy stats must be updated atomically. Do not use read-modify-write in Python for counters. Use atomic SQL increments.
...
Money columns use `NUMERIC(18,4)`.

Append-heavy timestamp tables should be partitioned by month before serious production volume.
...
↓
Worker acquires match lock
↓
Worker schedules Scrapyd spider with workspace_id, scrape_job_id, match_ids
↓
Scrapy spider loads match + variant + competitor
↓
...
or Redis lock: lock:scheduler:refresh-rules
```
...
Partitioning can be implemented after MVP but before real production volume.
...
Optional defense-in-depth after MVP:
...
Postgres and Redis run locally.
GET /health works.
...
base model patterns
core enums
Decimal/NUMERIC money rule
workspace-scoped model base
```
...
scrape_jobs
scrape_job_targets
worker calls Scrapyd schedule API
run one match
run one variant
...
```text
Redis token bucket or sliding-window limiter
...
Purpose: protect Postgres growth.

Covers:

```text
monthly partitions
price_observations partitioning
request_attempts partitioning
webhook_events partitioning
price_alert_events partitioning
retention job
daily rollups
...
```text
Next month partitions can be created.
Old partitions can be dropped according to retention.
...
DB-driven configuration
Scrapyd-managed Scrapy services
no Scrapy inside Celery tasks
